...
// xmlhttp.send("x=" + dbParam); 
// }
function index_pijat() {
   

obj = { "table":"pijat", "limit":20 };
dbParam = JSON.stringify(obj);
let txt = "";
xmlhttp = new XMLHttpRequest();
xmlhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
        ...
        txt += "<div class='row text-center'>"
            for (x in myObj) {
                ...
                txt += "<div class='card'>"
            if (myObj[x].gambar == '') {
               txt += " <img class='card-img-top' src='assets/images/user.png'>";
            }else{
              txt += " <img class='card-img-top' src='mod/uploads/"+myObj[x].gambar+"'>";
            }
            
            txt += " <div class='card-body'>"
            txt += " <h3><a>" + myObj[x].nama + "</a></h3>";
            txt +=" <h4>"+ myObj[x].service + "</h4>"
           ...
           txt += " </div><div class='card-footer'><a data-toggle='modal' data-target='#psn-pjt-Modal' onclick='pesan_pjt("+myObj[x].id_pemijat+");' class='btn btn-primary'>Pesan</a> </div></div></div>"
            }
        document.getElementById("data").innerHTML = txt;
...
xmlhttp.open("POST", "mod/pijat.php?act=search&cari=&sel=all",true);
xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
xmlhttp.send("x=" + dbParam);
}
$(document).ready(function(){
        ...
        $.ajax({
            ...
            success:function(response){
                ...
                $("#sel_user").empty();
                for( var i = 0; i<len; i++){
                    var id = response[i]['id_serv'];
                    var name = response[i]['service'];
...
$(document).on("submit","#formTambahPijat",function(e){
  e.preventDefault();
